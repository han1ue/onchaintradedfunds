...
import { IUniswapUniversalRouter, IUniswapV4StateView, IPermit2AllowanceTransfer } from "./interfaces/IUniswapV4.sol";
...
contract UniswapUniversalRouterAdapter is ITradeAdapter, ReentrancyGuard {
    ...
    constructor(address router, address factory, bytes32 poolHash, address manager, address stateView, address universal, address permit) {
        _contract(router);
        _contract(factory);
        _contract(manager);
        _contract(stateView);
        ...
        _contract(wrapped);
        if (poolHash == bytes32(0) || IUniswapV4StateView(stateView).poolManager() != manager
            || IUniswapUniversalRouter(universal).poolManager() != manager) revert InvalidDependency(universal);
    ...
    function executeBatch(Trade[] calldata trades, address[] calldata tokens, uint256[] calldata funding, uint256 deadline)
        external nonReentrant returns (uint256[] memory returned)
    {
        ...
        if (tokens.length != required.length || funding.length != tokens.length) revert InvalidPlan();
        if (uniswapUniversalRouter.codehash != routerCodehash
            || IUniswapV4StateView(uniswapV4StateView).poolManager() != uniswapV4PoolManager
            || IUniswapUniversalRouter(uniswapUniversalRouter).poolManager() != uniswapV4PoolManager) revert InvalidDependency(uniswapUniversalRouter);
        ...
        Commands memory plan = Commands(new bytes(4 * MAX_TRADES + 2 * MAX_TOKENS + 5), new bytes[](4 * MAX_TRADES + 2 * MAX_TOKENS + 5), 0);
        ...
        bool usesWeth;
        for (uint256 i; i < n; ++i) if (tokens[i] == weth) usesWeth = true;
        ...
        for (uint256 i; i < trades.length; ++i) {
            ...
            if (amount == type(uint256).max) minimum[input] = 0;
            else {
                if (amount > minimum[input]) revert InsufficientBalance(trade.tokenIn, minimum[input], amount);
                minimum[input] -= amount;
            }
            minimum[output] += trade.minAmountOut;
            bool finalOutput = true;
            for (uint256 j = i + 1; j < trades.length; ++j) {
                if (trades[j].tokenIn == trade.tokenOut) { finalOutput = false; break; }
            }
            if (finalOutput) directMinimum[output] += trade.minAmountOut;
            _swap(plan, trade, finalOutput ? entryExitRouter : THIS);
        }
        for (uint256 i; i < n; ++i) _append(plan, 0x04, abi.encode(tokens[i], entryExitRouter, minimum[i] - directMinimum[i]));
        ...
        if (usesWeth && nativeReserve != 0) {
            ...
            // The uint160 bound is checked above; Permit2 expires at the current timestamp.
            // forge-lint: disable-next-line(unsafe-typecast)
            IPermit2AllowanceTransfer(permit2).approve(weth, uniswapUniversalRouter, uint160(nativeReserve), uint48(block.timestamp));
        ...
        assembly ("memory-safe") { mstore(codes, count) mstore(inputs, count) }
        ...
        if (usesWeth && nativeReserve != 0) {
            ...
            (uint160 allowance,,) = IPermit2AllowanceTransfer(permit2).allowance(address(this), weth, uniswapUniversalRouter);
            if (allowance != 0 || IERC20(weth).allowance(address(this), permit2) != 0) revert InvalidPlan();
        }
        if (uniswapUniversalRouter.balance != nativeReserve) revert BalanceMismatch(address(0), nativeReserve, uniswapUniversalRouter.balance);
        ...
        for (uint256 i; i < n; ++i) {
            ...
            if (balance < routerBefore[i] + minimum[i]) revert InsufficientBalance(tokens[i], balance, routerBefore[i] + minimum[i]);
    ...
    function _swap(Commands memory plan, Trade calldata trade, address recipient) private view {
        ...
        if (data[0] == 0x03) {
            ...
            for (uint256 i; i < hops; ++i) _v3Pool(UniswapV3Path.tokenAt(path, i), UniswapV3Path.tokenAt(path, i + 1), UniswapV3Path.feeAt(path, i));
            _append(plan, 0x00, abi.encode(recipient, trade.amountIn == type(uint256).max ? CONTRACT_BALANCE : trade.amountIn,
                trade.minAmountOut, path, false, new uint256[](0)));
        } else {
            ...
            for (uint256 i; i < path.length; ++i) {
                ...
                if ((hop.fee > 1_000_000 && hop.fee != 0x800000) || hop.tickSpacing <= 0 || hop.tickSpacing > 32_767
                    || hop.hookData.length > 1_024 || (address(hop.hooks) != address(0) && address(hop.hooks).code.length == 0)) revert InvalidPath();
                (address a, address b) = current < next ? (current, next) : (next, current);
                (uint160 price,,,) = IUniswapV4StateView(uniswapV4StateView).getSlot0(keccak256(abi.encode(a, b, hop.fee, hop.tickSpacing, hop.hooks)));
                if (price == 0) revert UnauthenticatedPool();
                current = next;
            }
            if (trade.minAmountOut > type(uint128).max || (trade.amountIn != type(uint256).max && trade.amountIn > type(uint128).max)) revert InvalidPlan();
            if (currencyIn == address(0)) _append(plan, 0x0c, abi.encode(THIS, 0));
            bytes[] memory params = new bytes[](4);
            params[0] = abi.encode(currencyIn, trade.amountIn == type(uint256).max ? CONTRACT_BALANCE : trade.amountIn, false);
            params[1] = abi.encode(IV4Router.ExactInputParams(Currency.wrap(currencyIn), path, new uint256[](0), 0, uint128(trade.minAmountOut)));
            ...
            if (currencyIn == address(0) || current == address(0)) _append(plan, 0x0b, abi.encode(THIS, CONTRACT_BALANCE));
        }
    }
    ...
    function _tokens(Trade[] calldata trades) private view returns (address[] memory tokens) {
        ...
        for (uint256 i; i < trades.length; ++i) {
            Trade calldata trade = trades[i];
            if (trade.amountIn == 0 || trade.minAmountOut == 0 || trade.minAmountOut > type(uint128).max
                || (trade.amountIn != type(uint256).max && trade.amountIn > type(uint128).max)
                || trade.tokenIn == trade.tokenOut || trade.data.length == 0) revert InvalidPlan();
            bytes calldata data = trade.data;
            address current;
            if (data[0] == 0x03) {
                bytes calldata path = data[1:];
                ...
                (address currencyIn, PathKey[] memory path) = abi.decode(data[1:], (address, PathKey[]));
                if (path.length == 0 || path.length > MAX_HOPS || _boundary(currencyIn) != trade.tokenIn) revert InvalidPath();
                current = currencyIn;
    ...
    function _v3Pool(address a, address b, uint24 fee) private view {
        if (a > b) (a, b) = (b, a);
        address expected = address(uint160(uint256(keccak256(abi.encodePacked(hex"ff", uniswapV3Factory, keccak256(abi.encode(a, b, fee)), v3PoolInitCodeHash)))));
        ...
        if (p.factory() != uniswapV3Factory || p.token0() != a || p.token1() != b || p.fee() != fee) revert UnauthenticatedPool();
    }

    function _add(address[] memory tokens, uint256 count, address token) private view returns (uint256) {
        _contract(token);
        for (uint256 i; i < count; ++i) if (tokens[i] == token) return count;
        tokens[count] = token;
        return count + 1;
    }
    ...
    function _boundary(address token) private view returns (address) { return token == address(0) ? weth : token; }
    function _contract(address account) private view { if (account.code.length == 0) revert InvalidDependency(account); }
