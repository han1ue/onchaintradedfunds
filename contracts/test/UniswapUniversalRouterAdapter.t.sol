...
contract BatchV3Pool {
    ...
    constructor() {
        factory = msg.sender;
        (token0, token1, fee) = BatchV3Factory(msg.sender).parameters();
    }

    function swap(address recipient, bool zeroForOne, int256 amount, uint160, bytes calldata data)
        external
...
contract BatchV3Factory {
    ...
    function parameters() external view returns (address, address, uint24) {
        return (a, b, f);
    }

    function getPool(address x, address y, uint24 fee) external view returns (address) {
        return pools[_key(x, y, fee)];
    }
    ...
    function createPool(address x, address y, uint24 fee) external returns (address pool) {
        ...
        (a, b) = x < y ? (x, y) : (y, x);
        f = fee;
        pool = address(new BatchV3Pool{ salt: key }());
        pools[key] = pool;
    }

    function _key(address x, address y, uint24 fee) private pure returns (bytes32) {
        return x < y ? keccak256(abi.encode(x, y, fee)) : keccak256(abi.encode(y, x, fee));
    }
}
...
contract UniswapUniversalRouterAdapterTest is AtomicRouterTestBase {
    ...
    function setUp() public {
        ...
        universal = new UniversalRouter(
            RouterParameters(
                ...
                address(factory),
                bytes32(0),
                keccak256(type(BatchV3Pool).creationCode),
                address(manager),
                address(0),
                address(0),
                address(0)
        ...
        adapter = new UniswapUniversalRouterAdapter(
            address(router),
            address(factory),
            keccak256(type(BatchV3Pool).creationCode),
            address(manager),
    ...
    function testBatchedV4AndNativeExit() public {
        ...
        BasketRedeemRequest memory request =
            BasketRedeemRequest(address(targetVault), address(weth), shares, 1, 0, block.timestamp);
    ...
    function testUnauthorizedAndMinimumFailureAreAtomic() public {
        ...
        trades[0] = ITradeAdapter.Trade(
            address(input),
            address(assetC),
            ONE,
            ONE,
            _v3(address(input), address(assetC), ONE, ONE).data
        );
    ...
    function testIndependentPairsAndMultihopShareOneBatch() public {
        ...
        trades[0] = ITradeAdapter.Trade(
            ...
            ONE,
            ONE,
            abi.encodePacked(hex"03", address(input), FEE, address(assetA), FEE, address(assetC))
        );
        trades[1] = ITradeAdapter.Trade(
            address(weth),
            address(assetD),
            2 * ONE,
            2 * ONE,
            _v3(address(weth), address(assetD), 2 * ONE, 2 * ONE).data
        );
        address[] memory tokens = adapter.routeTokens(trades);
    ...
    function testFuzzRepeatedTokenAndRoundingRefund(uint96 quantity, uint8 remainder) public {
        ...
        legs[0] =
            _v3(address(input), address(assetA), amount * 2 + remainder, amount * 2 + remainder);
        ...
        vm.prank(ALICE);
        (uint256 shares,,) =
            router.mintFromToken(_mintRequest(amount * 2 + remainder, amount), legs);
    ...
    function testMalformedEndpointUnknownPoolAndSentinelAmountsRevert() public {
        ...
        vm.prank(ALICE);
        vm.expectPartialRevert(UniswapUniversalRouterAdapter.UnauthenticatedPool.selector);
        router.mintFromToken(_mintRequest(ONE, 1), legs);
        legs[0] = _v3(address(input), address(assetC), 1 << 255, ONE);
        vm.prank(ALICE);
        vm.expectPartialRevert(UniswapUniversalRouterAdapter.InvalidPlan.selector);
        router.mintFromToken(_mintRequest(ONE, 1), legs);
        assertEq(targetVault.totalSupply(), 0);
    }
    ...
    function testSkippedIntermediateIsRejectedBeforeSettlement() public {
        ...
        legs[0] = SwapLeg(
            ...
            ONE,
            ONE,
            abi.encodePacked(hex"03", address(assetB), FEE, address(assetA), FEE, address(input))
        );
        BasketRedeemRequest memory request =
            BasketRedeemRequest(address(sourceVault), address(input), ONE, 1, 1, block.timestamp);
        vm.prank(ALICE);
        vm.expectRevert();
    ...
    function testBatchDeadlineRejectsBeforeExecution() public {
        ...
        address[] memory tokens = adapter.routeTokens(trades);
        vm.prank(address(router));
        vm.expectPartialRevert(UniswapUniversalRouterAdapter.DeadlineExpired.selector);
        adapter.executeBatch(trades, tokens, new uint256[](tokens.length), block.timestamp - 1);
    }

    function _nativeMint(uint256 amount) private view returns (BasketMintRequest memory request) {
        request = _mintRequest(amount, 1);
        request.inputToken = address(weth);
    }

    function _v3(address a, address b, uint256 amount, uint256 minimum)
        private
        view
        returns (SwapLeg memory)
    {
        return SwapLeg(
            address(adapter), a, b, amount, minimum, abi.encodePacked(hex"03", a, FEE, b)
        );
    }

    function _v4(address a, address b, uint256 amount, uint256 minimum)
        private
        view
        returns (SwapLeg memory)
    {
        ...
        return SwapLeg(
            address(adapter),
            a == address(0) ? address(weth) : a,
            b == address(0) ? address(weth) : b,
            amount,
            minimum,
            bytes.concat(hex"04", abi.encode(a, path))
        );
    }
    ...
    function _v4Pool(address a, address b) private {
        (a, b) = a < b ? (a, b) : (b, a);
        PoolKey memory key =
            PoolKey(Currency.wrap(a), Currency.wrap(b), 3000, 60, IHooks(address(0)));
        manager.initialize(key, uint160(1 << 96));
        PoolModifyLiquidityTest lp = new PoolModifyLiquidityTest(manager);
        if (a != address(0)) {
            MockStockToken(a).mint(address(this), 1000 * ONE);
            IERC20(a).approve(address(lp), type(uint256).max);
        }
        MockStockToken(b).mint(address(this), 1000 * ONE);
        IERC20(b).approve(address(lp), type(uint256).max);
        vm.deal(address(this), 1000 * ONE);
        lp.modifyLiquidity{ value: a == address(0) ? 1000 * ONE : 0 }(
