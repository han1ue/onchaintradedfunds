...
2. Set the variables in `.env.example`. `AUTH_SECRET`, `CRON_SECRET`, and `IP_HASH_SECRET` must be independent random secrets. Use the same `AUTH_SECRET` in Preview and Production so OAuth checks created before a redirect can be decoded after it. `ADMIN_X_IDS` contains immutable X IDs, not handles. `TWITTERAPI_IO_API_KEY` remains server-only.
3. Create an X OAuth 2.0 application with callback URL `/api/auth/callback/twitter` and scope `users.read`. X OAuth proves account ownership; TwitterAPI.io supplies the first-sign-in profile snapshot with immutable ID, verification, account age, public status, and follower count. Submissions and votes use free X intents plus oEmbed verification; the app never publishes posts through an API.
...
The configured daily cron schedule is compatible with Vercel Hobby. Set `NEXT_PUBLIC_SITE_URL=https://onchaintradedfunds-launch.vercel.app`, `TURNSTILE_HOSTNAMES=onchaintradedfunds-launch.vercel.app`, and the X callback to `https://onchaintradedfunds-launch.vercel.app/api/auth/callback/twitter`. Preview deployments automatically proxy OAuth callbacks through `VERCEL_PROJECT_PRODUCTION_URL`; `AUTH_REDIRECT_PROXY_URL=https://onchaintradedfunds-launch.vercel.app/api/auth` can be set explicitly when Vercel system environment variables are unavailable. Update these values together if a custom hostname is added later.
