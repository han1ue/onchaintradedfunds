...
export function BallotPanel({ proposals, ballot, eligibility, availability, focusSlug, turnstileSiteKey, siteUrl }: {
  ...
  const actionPanel = !challenge ? <SectionCard className="ballotAction ballotActionWide">
    ...
    <div className="ballotActionPublish"><div className="xPostPreview compact"><div><span>Post preview · {disclosurePreviewLabel}</span><Send size={13} /></div><p>{previewText}</p></div><Turnstile siteKey={turnstileSiteKey} action="vote_otf" resetKey={turnstileResetKey} onToken={setTurnstileToken} />{(!turnstileSiteKey || turnstileToken) && <div className="postAction"><Button onClick={() => request("prepare")} disabled={busy || newVotes < 1}>{busy ? "Preparing…" : newVotes > 0 ? `Prepare X post for ${newVotes} ${newVotes === 1 ? "vote" : "votes"}` : "Choose votes to continue"}</Button><p className="postAssurance">We never post anything on your behalf.</p></div>}</div>
  </SectionCard> : <SectionCard className="ballotAction ballotActionWide">
    <div className="ballotActionIntro"><strong>Publish your voting post</strong><p>This post verifies {newVotes} {newVotes === 1 ? "vote" : "votes"}. You can batch votes together, but every future voting action will require a new post.</p></div>
