...
import { ArrowDown, CalendarClock, CheckCircle2, CircleAlert, ExternalLink, LockKeyhole, Minus, Plus, Send, ShieldAlert, Vote, X } from "lucide-react";
...
function VotePostDialog({ challenge, open, postUrl, busy, newVotes, onPostUrlChange, onVerify, onClose }: {
  challenge: Challenge;
  open: boolean;
  postUrl: string;
  busy: boolean;
...
export function BallotPanel({ proposals, ballot, eligibility, availability, focusSlug, turnstileSiteKey, siteUrl }: {
  ...
  if (!proposals.length) return <SectionCard className="emptyState ballotEmptyState"><Vote size={28} /><h2>No OTF proposals available</h2><p>Your unlocked votes remain available until proposals join the competition.</p><Button href="/submit">Submit the first OTF</Button></SectionCard>;
  ...
  return <div className="ballotLayout"><SectionCard className="ballotCard"><div className="ballotToolbar"><div><span>Your vote ledger</span><small>Cast votes are permanent. Add newly unlocked votes at any time.</small></div><div className={`ballotTotal${newVotes > 0 ? " valid" : ""}`} aria-label={`${total} of ${availability.unlockedVotes} votes selected`}><strong>{total} / {availability.unlockedVotes}</strong></div></div>
    ...
    {challenge && <VotePostDialog challenge={challenge} open={postDialogOpen} postUrl={postUrl} busy={busy} newVotes={newVotes} onPostUrlChange={setPostUrl} onVerify={() => request("verify")} onClose={() => setPostDialogOpen(false)} />}
  </div>;
}
