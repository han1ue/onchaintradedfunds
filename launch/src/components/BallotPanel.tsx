...
export function BallotPanel({ proposals, ballot, eligibility, availability, focusSlug, turnstileSiteKey, siteUrl }: {
  ...
  const previewText = buildVotePost(reason || "[Why you are participating in OTF Launch…]", siteUrl, "[verification code]", revealVotes ? addedChoices : []);
  ...
  </SectionCard><aside className="ballotRail"><SectionCard className="ballotSummary"><div><span>Unlocked and unassigned</span><strong className={unlockedRemaining === 0 ? "complete" : ""}>{unlockedRemaining}</strong></div><p>{availability.nextVoteUnlockAt ? `Next vote unlocks ${formatDateTime(availability.nextVoteUnlockAt)}.` : `All ${COMPETITION_RULES.totalVotes} votes are unlocked. No vote is added on voting day 30.`}</p><div className="voteUnlockTrack" role="progressbar" aria-label="Votes unlocked" aria-valuemin={0} aria-valuemax={COMPETITION_RULES.totalVotes} aria-valuenow={availability.unlockedVotes}><span style={{ width: `${availability.unlockedVotes / COMPETITION_RULES.totalVotes * 100}%` }} /></div><small>{availability.unlockedVotes} of {COMPETITION_RULES.totalVotes} unlocked</small></SectionCard>
    {!challenge ? <SectionCard className="ballotAction"><strong>{active ? "Post and cast new votes" : "Post and cast your first votes"}</strong><p>{newVotes > 0 ? `${newVotes} new ${newVotes === 1 ? "vote is" : "votes are"} ready. One X post can verify this whole batch.` : unlockedRemaining > 0 ? "Use the + controls to choose one or more votes. Every voting transaction requires a new X post." : availability.nextVoteUnlockAt ? "You have cast every vote currently unlocked." : "You have cast all 12 votes."}</p>{ballot?.proofUrl && <a className="inlineLink" href={ballot.proofUrl} target="_blank" rel="noreferrer">View first vote post <ExternalLink size={13} /></a>}<label className="formField"><span>Why are you voting?</span><textarea value={reason} onChange={(event) => setReason(event.target.value)} placeholder="Share why you’re helping choose the next OTFs…" rows={3} maxLength={120} disabled={newVotes < 1} /><small>{reason.length} / 120 characters · minimum 20</small></label><label className="privacyChoice"><input type="checkbox" checked={revealVotes} onChange={(event) => setRevealVotes(event.target.checked)} disabled={newVotes < 1} /><span><strong>Reveal my picks in this post</strong><small>{revealVotes ? "The post will name the OTFs and vote counts in this batch." : "Off by default. The OTFs receiving these votes will stay private."}</small></span></label><div className="xPostPreview compact"><div><span>Post preview · {revealVotes ? "batch picks included" : "picks not shown"}</span><Send size={13} /></div><p>{previewText}</p></div><Turnstile siteKey={turnstileSiteKey} action="vote_otf" resetKey={turnstileResetKey} onToken={setTurnstileToken} /><div className="postAction"><Button onClick={() => request("prepare")} disabled={busy || newVotes < 1 || reason.trim().length < 20 || Boolean(turnstileSiteKey && !turnstileToken)}>{busy ? "Preparing…" : newVotes > 0 ? `Prepare X post for ${newVotes} ${newVotes === 1 ? "vote" : "votes"}` : "Choose votes to continue"}</Button><p className="postAssurance">We never post anything on your behalf.</p></div></SectionCard> : <SectionCard className="ballotAction"><strong>Publish your voting post</strong><p>This post verifies {newVotes} {newVotes === 1 ? "vote" : "votes"}. You can batch votes together, but every future voting transaction will require a new post.</p><div className="xPostPreview compact"><div><span>Ready to publish</span><Send size={13} /></div><p>{challenge.postText}</p></div><div className="postAction"><a className="button buttonPrimary" href={challenge.intentUrl} target="_blank" rel="noreferrer">Open X and post <ExternalLink size={14} /></a><p className="postAssurance">We never post anything on your behalf.</p></div><label className="formField"><span>X post URL</span><input value={postUrl} onChange={(event) => setPostUrl(event.target.value)} placeholder="https://x.com/yourname/status/…" inputMode="url" /></label><Button onClick={() => request("verify")} disabled={busy || !postUrl.trim()}>{busy ? "Verifying…" : `Verify post and cast ${newVotes} ${newVotes === 1 ? "vote" : "votes"}`}</Button><Button variant="ghost" onClick={() => { setChallenge(null); setPostUrl(""); }}>Start again</Button></SectionCard>}
