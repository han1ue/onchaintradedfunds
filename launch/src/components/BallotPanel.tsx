...
export function BallotPanel({ proposals, ballot, eligibility, availability, focusSlug, turnstileSiteKey, siteUrl }: {
  ...
  const actionPanel = !challenge ? <SectionCard className="ballotAction ballotActionWide">
    ...
    <div className="ballotActionPublish"><div className="xPostPreview compact"><div><span>Post preview · {disclosurePreviewLabel}</span><Send size={13} /></div><p>{previewText}</p></div><Turnstile siteKey={turnstileSiteKey} action="vote_otf" resetKey={turnstileResetKey} onToken={setTurnstileToken} /><div className="postAction"><Button onClick={() => request("prepare")} disabled={busy || newVotes < 1 || reason.trim().length < 20 || Boolean(turnstileSiteKey && !turnstileToken)}>{busy ? "Preparing…" : newVotes > 0 ? `Prepare X post for ${newVotes} ${newVotes === 1 ? "vote" : "votes"}` : "Choose votes to continue"}</Button><p className="postAssurance">We never post anything on your behalf.</p></div></div>
