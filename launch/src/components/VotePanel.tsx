...
export function VotePanel({
  ...
  castVotes: number;
  availability: { votingOpen: boolean; competitionEnded: boolean; unlockedVotes: number; votingStartsAt: string };
}) {
  const availableVotes = Math.max(0, availability.unlockedVotes - castVotes);
  if (availability.competitionEnded) return <SectionCard className="votePanel"><div className="otfVoteTotal"><span>OTF votes</span><strong>{totalVotes.toLocaleString()}</strong></div><div className="cardHeading"><div><span>Your votes</span><small>Voting closed</small></div><Vote size={19} /></div><div className="panelBody ballotCta"><div><strong>{allocatedVotes > 0 ? `${allocatedVotes} locked ${allocatedVotes === 1 ? "vote" : "votes"}` : "No votes cast"}</strong><p>Cast votes and their X post history remain available for the final audit.</p></div><Link className="button buttonSecondary" href="/me">View vote history</Link></div></SectionCard>;
