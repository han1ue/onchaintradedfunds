...
export function VotePanel({
  ...
  return <SectionCard className="votePanel"><div className="otfVoteTotal"><span>OTF votes</span><strong>{totalVotes.toLocaleString()}</strong></div><div className="cardHeading"><div><span>Your votes</span><small>{availability.votingOpen ? `${availableVotes} available` : "Voting begins after the submission week"}</small></div><Vote size={19} /></div><div className="panelBody ballotCta">
    <div><strong>{!availability.votingOpen ? "Voting opens on competition day 8" : allocatedVotes > 0 ? `${allocatedVotes} locked ${allocatedVotes === 1 ? "vote" : "votes"}` : `Vote for ${proposal.name}`}</strong><p>{availability.votingOpen ? "Once cast, a vote cannot be moved or removed" : `Three votes unlock at ${new Date(availability.votingStartsAt).toLocaleDateString("en-US", { month: "short", day: "numeric" })}. You can keep creating OTFs after voting opens.`}</p></div>
