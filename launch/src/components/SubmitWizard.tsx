...
export function SubmitWizard({ competition, assets, eligibility, initialDraft = null, confirmedProposalCount, turnstileSiteKey }: {
  ...
  return <div className="wizardLayout"><SectionCard className="wizardCard">
    ...
    <div className="wizardFooter"><Button variant="secondary" onClick={() => challenge ? setChallenge(null) : setStep((current) => Math.max(1, current - 1))} disabled={step === 1 || busy || profileRecovery}>{challenge ? "Start again" : "Back"}</Button>{step < 4 ? <Button onClick={() => setStep((current) => current + 1)} disabled={step === 1 ? !name.endsWith(" OTF") || ticker.length < 1 || !thesisValid : step === 2 ? !allAssetsSelected || !allPricingReady || total !== 100 || !assetsUnique || !allWeightsMeetMinimum : false}>Continue</Button> : <Button onClick={postSubmission} disabled={busy || preview || !challenge || !draftId || !validPostUrl || profileRecovery}>{busy ? "Submitting…" : "Submit OTF"}</Button>}</div>
