...
export function SubmitWizard({ competition, assets, eligibility, turnstileSiteKey, siteUrl }: { competition: CompetitionSummary; assets: EligibleAsset[]; eligibility: ParticipationEligibility; turnstileSiteKey?: string; siteUrl: string }) {
  ...
  const postText = buildSubmissionPost(reason || "[This OTF is great because…]", { name: name || "Your OTF", ticker: ticker || "TICKER", slug: slugifyProposalName(name || "Your OTF") }, siteUrl, "[verification code]");
  ...
  return <div className="wizardLayout"><SectionCard className="wizardCard">
    ...
    <div className="wizardBody">
      ...
      {step === 2 && <div className="formStack">{!hasEnoughAssets ? <Callout tone="danger">No portfolio assets are available. At least two assets must be added before proposals can be submitted. <Link className="inlineLink" href="/rwas">View all supported RWAs</Link>.</Callout> : <><div><div className="allocationTotal"><span>Portfolio allocation</span><strong className={total === 100 ? "valid" : ""}>{total}%</strong></div><p className="assetDirectoryPrompt">Build your portfolio from supported assets. <Link className="inlineLink" href="/rwas">View all supported RWAs</Link>.</p></div>{rows.map((row, index) => <div className="allocationInput" key={index}><label className="formField"><span>Asset {index + 1}</span><select value={row.assetId} onChange={(event) => updateRow(index, { assetId: event.target.value })}>{assets.map((asset) => <option key={asset.id} value={asset.id}>{asset.symbol} · {asset.name}</option>)}</select></label><label className="formField weightField"><span>Weight</span><div><input type="number" min="1" max="99" value={row.weight} onChange={(event) => updateRow(index, { weight: Number(event.target.value) })} /><span>%</span></div></label>{rows.length > 2 && <button className="removeButton" type="button" onClick={() => setRows((current) => current.filter((_, itemIndex) => itemIndex !== index))} aria-label={`Remove asset ${index + 1}`}><Trash2 size={16} /></button>}</div>)}<Button variant="secondary" onClick={() => setRows((current) => [...current, { assetId: assets.find((asset) => !current.some((row) => row.assetId === asset.id))?.id ?? assets[0]?.id ?? "", weight: 1 }])} disabled={rows.length >= assets.length}><Plus size={15} /> Add asset</Button></>}</div>}
