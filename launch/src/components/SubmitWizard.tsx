...
export function SubmitWizard({ competition, assets, eligibility, turnstileSiteKey, siteUrl }: {
  ...
  return <div className="wizardLayout"><SectionCard className="wizardCard">
    ...
    <div className="wizardBody">
      ...
      {step === 2 && <div className="formStack">{verifiedAssets.length === 0 && <Callout>No verified assets are available yet. Search for a Robinhood Chain contract address; the server will validate its 18-decimal ERC-20 and qualifying Uniswap V3 market.</Callout>}<div><div className="allocationTotal"><span>Portfolio allocation · {rows.length} assets <small>(2 minimum)</small></span><strong className={total === 100 ? "valid" : ""}>{total}%</strong></div><p className="assetDirectoryPrompt">Choose from verified assets; their saved price sources are already configured. If an asset is not listed, enter its contract and pool address. The Add token action stays locked until every observed requirement passes.</p></div><div className="allocationRows"><div className="allocationColumnHeaders"><span>Assets</span><span>Weight</span><span aria-hidden="true" /></div>{rows.map((row, index) => <div className="allocationInput" key={index}><AssetMarketPicker assets={verifiedAssets} assetId={row.assetId} assetMetadata={row.assetMetadata} pricingConfig={row.pricingConfig} label={`Asset ${index + 1}`} onChange={(assetId, assetMetadata, pricingConfig) => updateRow(index, { assetId, assetMetadata, pricingConfig })} /><label className="formField weightField"><span className="srOnly">Asset {index + 1} weight</span><div><input aria-label={`Asset ${index + 1} weight percentage`} type="text" inputMode="numeric" pattern="[0-9]*" maxLength={2} value={row.weight} onChange={(event) => updateRow(index, { weight: normalizeWholeNumberInput(event.target.value, 99) })} /><span>%</span></div></label>{rows.length > 2 && <button className="removeButton" type="button" onClick={() => setRows((current) => current.filter((_, itemIndex) => itemIndex !== index))} aria-label={`Remove asset ${index + 1}`}><Trash2 size={16} /></button>}</div>)}</div><Callout tone="positive"><strong>Verified-only OTFs get a 100% boost on performance XP.</strong></Callout><Button variant="secondary" onClick={() => setRows((current) => { const asset = verifiedAssets.find((candidate) => !current.some((row) => row.assetId === candidate.id)); return [...current, initialRow(asset, "1")]; })}><Plus size={15} /> Add another asset</Button></div>}
      {step === 3 && <div className="reviewBlock">{quality === "high" && <div className="reviewBoost"><StatusBadge tone="positive">100% XP boost</StatusBadge></div>}<div><span>Name</span><strong>{name}</strong></div><div><span>Ticker</span><strong>${ticker}</strong></div><div><span>Thesis</span><p>{thesis}</p></div><div><span>Portfolio</span><ul>{rows.map((row, index) => { const asset = assets.find((candidate) => candidate.id === row.assetId); const metadata = asset ?? row.assetMetadata; return <li key={`${rowIdentities[index]}:${index}`}><span>{metadata?.symbol} · {metadata?.contractAddress}{row.pricingConfig ? ` · ${pricingConfigSummary(row.pricingConfig)}` : ""}</span><strong>{row.weight}%</strong></li>; })}</ul></div></div>}
      {step === 4 && <div className="proofFlow">{!challenge && <label className="formField"><span>Your context <small>(optional)</small></span><textarea value={reason} onChange={(event) => setReason(event.target.value)} placeholder="Why this OTF deserves to launch…" rows={3} maxLength={120} /><small>{reason.length} / 120 characters</small></label>}<div className="xPostPreview"><div><span>{challenge ? "Ready to publish" : "Post preview"}</span><Send size={14} /></div><p>{challenge?.postText ?? postText}</p></div>{!challenge && <><Callout>We’ll add a one-time verification code when you open X. After publishing, return here and paste the post URL below.</Callout><Turnstile siteKey={turnstileSiteKey} action="submit_otf" resetKey={turnstileResetKey} onToken={setTurnstileToken} /></>}{(challenge || !turnstileSiteKey || turnstileToken) && <div className="postAction">{challenge ? <a className="button buttonPrimary" href={challenge.intentUrl} target="_blank" rel="noreferrer">Open X and post <ExternalLink size={14} /></a> : <Button onClick={postSubmission} disabled={busy || preview}>{busy ? "Opening X…" : "Open X and post"} <ExternalLink size={14} /></Button>}<p className="postAssurance">We never post anything on your behalf.</p></div>}<label className="formField"><span>X post URL</span><input value={postUrl} onChange={(event) => setPostUrl(event.target.value)} placeholder="https://x.com/yourname/status/…" inputMode="url" disabled={!challenge || busy} /><small>{challenge ? "Paste the URL of the public post containing the verification code." : "Open X and publish the prepared post first; this field will then be ready."}</small></label></div>}
      {message && <p className="formMessage" role="status">{message}</p>}
    </div>
    <div className="wizardFooter"><Button variant="secondary" onClick={() => challenge ? setChallenge(null) : setStep((current) => Math.max(1, current - 1))} disabled={step === 1 || busy}>{challenge ? "Start again" : "Back"}</Button>{step < 4 ? <Button onClick={() => setStep((current) => current + 1)} disabled={step === 1 ? !name.endsWith(" OTF") || ticker.length < 1 || !thesisValid : step === 2 ? !allAssetsSelected || !allPricingReady || total !== 100 || !assetsUnique : false}>Continue</Button> : challenge ? <Button onClick={postSubmission} disabled={busy || preview || !postUrl.trim()}>{busy ? "Creating…" : "Create OTF"}</Button> : null}</div>
  </SectionCard><aside><SectionCard className="sideNote"><strong>Before you create an OTF</strong><ul><li>Use a verified, public X account with at least {eligibility.minFollowers.toLocaleString()} followers.</li><li>One OTF proposal per X account.</li><li>Choose at least two 18-decimal assets.</li><li>Verified assets already include an approved saved price source.</li><li>Unlisted assets require a canonical Uniswap V3 pool plus passing liquidity, verified market-cap, age, GT, honeypot, and locked-liquidity evidence.</li><li>Weights must total exactly 100%.</li><li>Created proposals cannot be edited.</li></ul></SectionCard></aside></div>;
}
