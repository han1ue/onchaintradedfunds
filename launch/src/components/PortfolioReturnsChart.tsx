...
export function PortfolioReturnsChart({ returns, preview = false }: { returns: PortfolioReturns; preview?: boolean }) {
  ...
  return <div className="portfolioReturns">
    <div className="returnsHeading">
      <div><h3>Returns since proposed</h3><p>Initial portfolio weights, indexed from the first complete Robinhood bid observation.</p></div>
    ...
    {points.length < 2 || !layout || !active ? <div className="returnsEmpty">
      ...
      <div className="returnsFootnote"><span>{preview ? "Preview series" : "Hourly bid snapshots"}</span><span>Baseline {formatDateTime(returns.trackingStartedAt ?? points[0].timestamp)}</span></div>
    </>}
  </div>;
}
