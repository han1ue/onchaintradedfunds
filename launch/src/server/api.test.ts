...
describe("public list query validation", () => {
  it("uses the bounded default and parses a stable rank cursor", () => {
    expect(parsePublicListQuery(new Request("https://launch.example/api/v1/leaderboard?cursor=50&q=AI"))).toEqual({
  ...
  it("accepts the hard maximum", () => {
    expect(parsePublicListQuery(new Request(`https://launch.example/api/v1/leaderboard?limit=${MAX_PUBLIC_LIST_LIMIT}`)).limit).toBe(MAX_PUBLIC_LIST_LIMIT);
  });

  it.each(["0", "101", "2.5", "nope"])("rejects invalid limit %s", (limit) => {
    expect(() => parsePublicListQuery(new Request(`https://launch.example/api/v1/leaderboard?limit=${limit}`))).toThrow("INVALID_QUERY");
  });
