...
const challengeSnapshot = JSON.parse(readFileSync(
...
describe("squashed migration metadata", () => {
  it("tracks the current schema migrations", () => {
    expect(journal.entries).toEqual([
      ...
      expect.objectContaining({ idx: 8, tag: "0008_challenge_results" }),
    ]);
    expect(journal.entries.every((entry: { when: number }, index: number, entries: Array<{ when: number }>) => index === 0 || entries[index - 1]!.when < entry.when)).toBe(true);
    ...
    expect(challengeSnapshot.prevId).toBe(invariantSnapshot.id);
    ...
    expect(challengeSnapshot.tables["public.x_action_challenges"].columns.result_ballot_id.type).toBe("uuid");
    expect(challengeSnapshot.tables["public.x_action_challenges"].columns.result_slug.type).toBe("text");
