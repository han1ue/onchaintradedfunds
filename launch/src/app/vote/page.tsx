...
import { getCompetition, getLeaderboard, getProposal } from "@/server/data";
...
export default async function VotePage({ searchParams }: { searchParams: Promise<{ focus?: string }> }) {
  ...
  const [competition, initialPage, session, focusedProposal] = await Promise.all([
    getCompetition(),
    getLeaderboard(),
    auth(),
    params.focus ? getProposal(params.focus) : null,
  ]);
  if (focusedProposal && !initialPage.entries.some((proposal) => proposal.id === focusedProposal.id)) {
    initialPage.entries = [...initialPage.entries, focusedProposal].sort((a, b) => a.rank - b.rank);
  }
