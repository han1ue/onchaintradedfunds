...
export default async function SubmitPage() { const [competition, assets, session] = await Promise.all([getCompetition(), getEligibleAssets(), auth()]); const eligibility = await getParticipationEligibility(session?.user, competition); return <div className="pageShell contentPage wideContent"><header className="pageHeader"><h1>Submit OTF proposal</h1><p>Build a coherent proposal from eligible real-world assets. The OTF proposal with the most verified votes ranks first on the leaderboard.</p></header><SubmitWizard competition={competition} assets={assets} eligibility={eligibility} turnstileSiteKey={process.env.NEXT_PUBLIC_TURNSTILE_SITE_KEY} siteUrl={process.env.NEXT_PUBLIC_SITE_URL ?? "http://localhost:3001"} /></div>; }
