...
import { BadgeCheck, CircleAlert, ExternalLink, Trash2 } from "lucide-react";
...
export default async function ProposalPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  if (!proposal) {
    ...
    return <InvalidProposalState proposal={invalidProposal} canDelete={false} />;
  }
  ...
  const canDelete = session?.user.xUserId === proposal.creator.xId && proposal.votes === 0;
