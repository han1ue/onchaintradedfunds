...
export default async function ProposalPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <div className="pageShell proposalPage"><section className="proposalHero"><div className="proposalTitle"><OtfTokenIcon ticker={proposal.ticker} size={64} /><div><h1>{proposal.name}</h1><div className="proposalMeta"><StatusBadge tone="positive">Rank #{proposal.rank}</StatusBadge><StatusBadge tone={proposal.verified ? "positive" : "neutral"}>{proposal.verified ? "Verified" : "Non-verified"}</StatusBadge><time dateTime={proposedAt.toISOString()} title={proposedAt.toLocaleString("en-US", { dateStyle: "medium", timeStyle: "short", timeZone: "UTC" })}>{formatProposalAge(proposedAt)}</time></div></div></div><div className="creatorBlock"><span>Created by</span><strong><XProfileImage src={proposal.creator.profileImageUrl} username={proposal.creator.username} size={30} />@{proposal.creator.username}{proposal.creator.verified && <BadgeCheck className="xVerifiedBadge" size={16} aria-label="Verified X account" />}</strong></div></section>
    <Callout>{proposal.verified
      ? <><strong>Verified OTF.</strong> Every constituent is currently verified, so votes for this OTF compete for the 3,500,000 XP verified-performance pool. Individual awards depend on relative score.</>
