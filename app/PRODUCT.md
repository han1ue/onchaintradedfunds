...
The thesis, constituents, and raw units go onchain. Prices, market caps, percentages, target value, and weighting method stay offchain. After confirming `VaultCreated`, the browser saves the weighting snapshot by chain and vault. Fund details show each constituent’s name, ticker, verification badge, and accounted balance in units, including zero balances for empty vaults. The allocation table always calculates allocation percentages from the contract’s initial basket quantities at current prices, while amounts show current accounted balances. Dollar prices and the market-cap comparison reuse the NAV pricing response with no additional price requests. A current market-cap match allows up to 0.01 percentage points difference per constituent; it does not classify the original launch settings.
