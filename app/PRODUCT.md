...
The thesis, constituents, and raw units go onchain. Prices, market caps, percentages, target value, and weighting method stay offchain. After confirming `VaultCreated`, the browser saves the weighting snapshot by chain and vault. Fund details show each constituent’s name, ticker, verification badge, and accounted balance in units, including zero balances for empty vaults. The allocation table reuses the NAV pricing response for dollar prices and allocation percentages based on those balances. When share supply is zero, allocation percentages use the contract’s initial basket quantities while amounts remain the actual balances. Neither case requires additional price requests. A current market-cap match allows up to 0.01 percentage points difference per constituent; it does not classify the original launch settings.
