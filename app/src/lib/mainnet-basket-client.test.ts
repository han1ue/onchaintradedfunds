...
import { mainnetBasketClient, type MainnetBasketDeployment } from "./mainnet-basket-client";
...
describe("mainnet basket simulation", () => {
  it("authenticates V4 pool keys through StateView and checks hook code", async () => {
    ...
    await mainnetBasketClient(deployment).authenticateV4Pool(addr(8), hop);
    ...
    await expect(mainnetBasketClient(deployment).authenticateV4Pool(addr(8), hop)).rejects.toThrow(/Uninitialized/);
    rpc.readContract.mockResolvedValue([1n, 0, 0, 3000]);
    rpc.getCode.mockResolvedValue("0x");
    await expect(mainnetBasketClient(deployment).authenticateV4Pool(addr(8), hop)).rejects.toThrow(/hook/);
  });
  it("simulates approvals and the complete basket in order against actual balances", async () => {
    ...
    expect(await mainnetBasketClient(deployment).simulate(execution())).toEqual({ amountOut: 20n, refunds: [], gasUsed: 123n });
  ...
  it("decodes native mint refunds separately from ERC20 refunds", async () => {
    ...
    expect(await mainnetBasketClient(deployment).simulate(execution(true))).toEqual({ amountOut: 20n, refunds: [{ token: addr(9), amount: 3n }, { token: zeroAddress, amount: 7n }], gasUsed: 100n });
  });
  it("rejects approval failures, reverted baskets, incomplete results, and unsupported RPCs", async () => {
    for (const results of [[], [{ status: "failure" }, { status: "success" }, { status: "success" }], [{ status: "success" }, { status: "success" }, { status: "failure" }]]) {
      rpc.simulateCalls.mockResolvedValue({ results });
      await expect(mainnetBasketClient(deployment).simulate(execution())).rejects.toThrow(/simulation/);
    }
    rpc.simulateCalls.mockRejectedValue(new Error("eth_simulateV1 unsupported"));
    await expect(mainnetBasketClient(deployment).simulate(execution())).rejects.toThrow(/unsupported/);
  });
});
