...
describe("$OTF page wiring", () => {
  ...
  it("routes bootstrap trades through the boundary router with partial-fill disclosure", () => {
    expect(operate).toContain("protocolDeploymentForChain(chainId)?.addresses.launchRouter");
  ...
  it("uses the fund header space for fund-specific metrics and gives the thesis the full hero width", () => {
    ...
    expect(css).toContain(".valuationChartWrap { min-height: 242px; padding: 18px 16px 22px;");
  });
  ...
  it("places the weekly distribution after directory AUM with a visible separator", () => {
    ...
    expect(summary).toContain("Week ${rewardsApy.week} rewards budget");
